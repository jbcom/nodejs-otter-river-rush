...
async function generateAnimation(
  ...
  if (await fs.pathExists(outputPath)) {
    console.log(
      chalk.yellow(
        `  ⚠️  otter-rusty-${animationName}.glb already exists, skipping`
      )
    );
...
async function main() {
  ...
  console.log(
    chalk.gray(
      'Generating additional animations from Meshy Animation Library\n'
    )
  );
  ...
  if (!rigTaskId) {
    console.log(
      chalk.yellow(
        '⚠️  rigTaskId not in manifest, looking up from animation URLs...\n'
      )
    );
    ...
    if (otterModel.animations && otterModel.animations[0]?.url) {
      ...
      if (match) {
        ...
        console.log(
          chalk.green(
            `✅ Extracted rigging task ID from URL: ${rigTaskId.substring(0, 12)}\n`
          )
        );
      }
    }
    ...
    console.log(
      chalk.green(
        `✅ Using rigging task ID from manifest: ${rigTaskId.substring(0, 12)}\n`
      )
    );
  }
  ...
  if (!rigTaskId) {
    ...
    console.log(
      chalk.yellow('Regenerate the otter model to save rigTaskId in manifest.')
    );
    process.exit(1);
  }
