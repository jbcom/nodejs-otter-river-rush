...
async function _downloadTexture(config: TextureConfig): Promise<void> {
  ...
  console.log(
    `   Note: ${config.ambientcgId} requires manual download from ambientcg.com`
  );
  console.log(`   URL: https://ambientcg.com/view?id=${config.ambientcgId}`);
  console.log(
    '   ⏭️ Skipping auto-download (would need unzip). Please download manually.'
  );
}
...
async function main() {
  ...
  console.log(
    'All textures are CC0 (public domain) from https://ambientcg.com\n'
  );
  ...
  for (const config of TEXTURES) {
    ...
    console.log(
      `  - Visit: https://ambientcg.com/view?id=${config.ambientcgId}`
    );
  ...
  console.log(
    '✨ Or use the canvas-based procedural textures already in BackgroundGenerator!'
  );
}
