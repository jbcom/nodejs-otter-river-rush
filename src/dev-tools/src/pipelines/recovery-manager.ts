...
export class RecoveryManager {
  ...
  async autoRecover(expectedModels: string[]): Promise<CompletionStatus> {
    ...
    try {
      ...
      for (const task of sortedTasks) {
        ...
        switch (task.status) {
          ...
          case 'PENDING':
            ...
            console.log(
              chalk.yellow(
                `  ⏳ In progress: ${matchedModel} (${task.progress}%)`
              )
            );
            break;
          ...
          case 'EXPIRED':
            console.log(
              chalk.red(`  ❌ Failed: ${matchedModel} (${task.status})`)
            );
            break;
        }
      }
      ...
      console.log(
        chalk.gray(
          `  🔗 Matched: ${matched} unique tasks (${duplicates} duplicates)`
        )
      );
      ...
      console.log(
        chalk.gray(`  🔁 Duplicate Tasks: ${duplicateTasks.length}\n`)
      );
      ...
      console.log(
        chalk.yellow(`⚠️  Could not complete recovery: ${error.message}`)
      );
